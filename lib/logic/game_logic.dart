...
class GameLogic extends ChangeNotifier {
  ...
  // Mevcut durumu kaydet (geri alma için)
  void _saveState() {
  ...
  // Kart koyulmadan önce kontrol et
  ...
  void markNeighborsDangerous(int row, int col) {
    _updateNeighbors(row, col, markAsDangerous: true);
    notifyListeners();
  }

  void markNeighborsSafe(int row, int col) {
    _updateNeighbors(row, col, markAsDangerous: false);
    notifyListeners();
  }

  void _updateNeighbors(int row, int col, {required bool markAsDangerous}) {
  ...
  void resetGame() {
    score = 0;
    remainingCards[CardValue.one] = 5;
    remainingCards[CardValue.two] = 2;
    remainingCards[CardValue.three] = 2;
    remainingCards[CardValue.four] = 1;
